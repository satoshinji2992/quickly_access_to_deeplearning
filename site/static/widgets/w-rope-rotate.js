  ...
  function mount(container) {
    container.innerHTML =
      ...
      '<div class="rr-row">' +
        ...
        '</div>' +
      '</div>' +
      '<p class="wg-note">把每个维度对 (x,y) 看成平面向量：位置 m 就是把它旋转 m·θi。θi = 10000^(−2i/D)，i 小转得快、i 大转得慢。单独旋转不改变各向量长度；两个向量旋转不同角度会改变夹角。固定原始 q、k 时，位置的影响通过相对距离进入点积，内容仍然影响打分。V 不旋转——位置只影响「query 与哪个 key 匹配」，不影响取回的内容。</p>';
