...
如果你在看完[课程主线]((/neural-networks-from-scratch/01-course-map/))中的实践引导还是觉得不好上手，不妨看看这个吧~
...
我们使用CE
损失函数：仅仅将交叉熵取均值即可。
(注意到：对于独热编码，交叉熵损失的计算就相当于-log(softmax(y_pred_true)),其中y_pred_true是预测正确那行的logit值）。）细节与计算方法详见[课程主线]((/neural-networks-from-scratch/01-course-map/))中的任务二实践引导。
<details><summary>这里是一种实现方式</summary>
...
具体的公式依旧详见[课程主线]((/neural-networks-from-scratch/01-course-map/)).
...
在[课程主线]((/neural-networks-from-scratch/01-course-map/))中，我们给出了对最后一层线性输出 $z$ 的导数为
...
这个代码被命名为data_creater,存储在 `exercises/task_01_circle_classifier/` 目录下。
...
接下来，不出意外的话，你通过命令行运行 `mission_1.py`：
...
![alt text](/projects/from-scratch-nn/assets/images/illustration_mission_1.png)
