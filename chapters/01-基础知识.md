...
旧稿里这一节还会讲一串训练时绕不开的东西: SiLU/GELU、Momentum、Adagrad、RMSProp、Adam、AdamW、BatchNorm、LayerNorm、Dropout、L2 正则化和 Kaiming 初始化。它们不是为了显得高级, 而是在回答训练里常见的问题: 为什么 loss 下降很慢, 为什么梯度抖得厉害, 为什么训练集很好测试集很差, 为什么同样学习率今天能收敛明天就发散。
