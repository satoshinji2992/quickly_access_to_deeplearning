...
数据与可视化约定:
- `starter.py` 每次运行都按当前 `condition` 重新生成 CSV, 不复用可能已经过期的标签.
- 训练/验证按类别分层为 800/200, 训练前检查标签、重复坐标和集合交集.
- 默认使用 He 初始化、`lr=0.1`、`batch_size=20`、`epochs=1000`, 每轮打乱数据.
- 日志同时记录训练与验证 loss/accuracy, 最后在验证点上画出预测类别.

可运行代码和预期结果见[圆形分类 `starter.py`](../exercises/block_01_basics/task_01_circle_classifier/README.md). 下面仍保留原稿的动手实践引导, 用来解释代码里的矩阵和梯度.
...
- 经典案例(二分类): Sigmoid + 交叉熵(BCE)
  ...
  这个值正好是预测值 − 真实值! 这是链式法则下 Sigmoid 与 BCE 导数相互抵消的结果, 沿链式法则展开即可得到.
...
#### 如何选择?
- ReLU 仍是 MLP/CNN 中简单可靠的基线; GELU 常见于早期与经典 Transformer, 现代模型也大量使用 SiLU/SwiGLU, 并不存在对所有网络都最好的单一激活函数.
