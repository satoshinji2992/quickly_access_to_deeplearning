...
上面的公式为了手算写成概率 $\mathbf{p}$. 圆形分类的 `Model.py` 也先显式计算 softmax, 再把概率交给 CE. 下一节封装后的 `CrossEntropyLoss` 改为直接接收 logits, 在内部完成稳定的 softmax. 两种接口都只计算一次 softmax, 不要混用.

本任务的明确选择(与后文保持一致)
- 输出层: 2 维 logits → softmax 概率.
- 损失: 交叉熵(CE, 接收概率, 对 batch 取均值).
- 隐藏层: ReLU.
...
这里沿用圆形分类 `Model.py` 的显式 softmax 写法. 到下一节的 `CrossEntropyLoss` 中, softmax 会移进损失函数内部, 届时传入的是 logits.
...
在进入 ResNet 之前, 当前课程先用同一套 Linear、ReLU、CrossEntropyLoss 和 Optimizer 完成一次 [MNIST MLP](../exercises/block_01_basics/task_03_mnist_mlp/README.md). 图片从 $(B,28,28)$ 展开到 $(B,784)$, 训练循环没有改变; flatten 丢失二维邻接关系的问题则自然引出下一章的卷积.
