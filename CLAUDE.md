...
**Interactive widgets:** `site/static/widgets/` hosts a vanilla-JS panel system (`panel.js` = right-side dock + lazy-loading registry; `widgets.css` = shared `.wg-*` primitives; `w-<id>.js` = one widget per file, registered via `DLWidget.register(id, mountFn)` where mountFn builds DOM in the panel and returns an optional dispose). Docs embed a trigger with a standalone HTML line `<div class="widget-mount" data-widget="<id>"></div>` (goldmark `unsafe=true` passes it through). New widgets must be added to the REGISTRY in panel.js; `tests/test_docs.py::test_widget_mounts_reference_registered_widgets` enforces marker↔registry↔file consistency. Deep link `docs/<slug>/?widget=<id>` opens a widget for QA screenshots. Plan and widget inventory: `docs/interactive_widgets_plan.md`.
