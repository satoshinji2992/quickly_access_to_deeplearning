...
最后是训练。这里不要一上来就跑完整 CIFAR-100, 先做小样本训练。如果几百张图片上 loss 都不下降, 不要急着调大模型, 先回去查数据、算子和训练循环。训练深度模型时, 这种小范围排错比盲目调参可靠得多。
...
| [task_14 NumPy ResNet 训练](../exercises/block_02_resnet/task_14_numpy_resnet_train/README.md) | 怎么把模块组装成训练项目 | 轻量 ResNet、日志、小样本训练                 |
